package vd.core.common;

public class Constant {

    // App data
    public final static String SEARCH_URL = "https://www.google.com/search?q=%s";

    // Google Play link
    public final static String GOOGLE_PLAY_LINK = "https://play.google.com/store/apps/details?id=%s";

    // Remote data
    public final static String REMOTE_URL = "";

    // Search url
    public final static String SUGGESTION_URL = "http://suggestqueries.google.com/complete/search?hl=en&ds=yt&client=youtube&hjson=t&cp=1&q=%s&format=5&alt=json";

    // Parser server
    public final static String PARSER_SERVER = "http://video-downloader11.herokuapp.com/api/info?url=%s";

    // App config
    public final static String UA_ID = "";

    public final static String AD_APP_ID = "";
    public final static String AD_BANNER_ID = "";
    public final static String AD_INTERSTITIAL_ID = "";
}
